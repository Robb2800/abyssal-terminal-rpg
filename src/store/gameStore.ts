...
export const useGameStore = create<GameState>((set, get) => ({
  ...
  selectOrigin: (originType) => {
    ...
    set({
      ...
      mana: 10 + (originData.statBonus.manaRegen ? 5 : 0),
      maxMana: 20 + (originData.statBonus.manaRegen ? 10 : 0),
      grid,
      position: { x: 2, y: 0 },
  ...
  attack: () => {
    const { currentEncounter, strength, triggerRoll, addLog, playerHp, defenseActive, agility, allies, roomsCleared, killCount, playerMaxHp, origin, bossPhase, relicCooldown } = get();
    ...
    triggerRoll('ATTACK', (roll) => {
      ...
      set({ currentEncounter: updatedEncounter, relicCooldown: Math.max(0, relicCooldown - 1) });
      if (updatedEncounter.hp <= 0) {
        ...
        if (updatedEncounter.name === 'ABYSSAL OVERSEER') {
          ...
          if (hpPct <= 10) {
            ...
            addLog(`>>> CRITICAL: SYSTEM_OVERRIDE INITIATED.`);
            return;
  ...
  defend: () => {
    const { relicCooldown } = get();
    set({ defenseActive: true, relicCooldown: Math.max(0, relicCooldown - 1) });
    get().addLog(`> DEFENSE: STANCE BRACED. RELIC CHARGING.`);
  },
  useRelic: () => {
    ...
    if (origin === 'collector') {
      ...
      set({ grid: newGrid, relicCooldown: 4 });
      addLog(`+ RELIC: SECTOR REGENERATED BY COMPASS.`);
    } else if (origin === 'exile') {
      if (!currentEncounter) return;
      set({ defenseActive: true, relicCooldown: 5 });
  ...
  useSkill: (id) => {
    const { mana, playerHp, playerMaxHp, addLog } = get();
    if (id === 'siphon') {
      if (mana < 5) {
        addLog(`! ERROR: INSUFFICIENT MANA.`);
        return;
      }
      set({ mana: mana - 5, playerHp: Math.min(playerMaxHp, playerHp + 5) });
      addLog(`+ SKILL: MANA SIPHON CONVERTED ESSENCE TO VITALITY.`);
    }
  },
  ...
  triggerRoll: (label, callback) => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => { 
        set(state => {
          if (state.lastRoll === roll) return { lastRoll: 0 };
          return {};
        });
      }, 2500);
    }, 1000);
  ...
  reset: () => set({
    ...
    restUsedOnFloor: false, currentAscii: ASCII_ART.VOID, lastRoll: 0, isRolling: false
  })
}));